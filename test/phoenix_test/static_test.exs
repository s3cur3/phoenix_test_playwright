defmodule PhoenixTest.StaticTest do
  ...
  describe "visit/2" do
    ...
    @tag skip: true, reason: :irrelevant
    ...
    @tag skip: true, reason: :known_inconsistency
  ...
  describe "click_button/2" do
    ...
    test "can handle clicking button that does not submit form after filling a form", %{
      conn: conn
    } do
      conn
      |> visit("/page/index")
  ...
  describe "select/3" do
    ...
    @tag skip: true, reason: :flaky_test
    test "can target a label with exact: false", %{conn: conn} do
      conn
      |> visit("/page/index")
      |> within("#complex-labels", fn session ->
